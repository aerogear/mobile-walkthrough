...
[time=15]
== Protect the GraphQL showcase app using the Red Hat SSO

In this task, you use the Red Hat SSO to protect both the client (PWA) and the data sync server.

=== Connect SSO to the client app
...
=== Connect Single SignOn to the server app

. Navigate to SSO realm to create new client.
...
.. Click on the *Installation* tab, and select `OIDC JSON` format. Use the `Download` button to save the configuration file to a directory on the computer.
